...
def novelty_analysis(corpus1, corpus2, motifs, path, corpus2Name, auto, dictionaries):
    ...
    worse = np.argsort(novelties[:, -1])[:5]
    ...
    for i in worse:
        ...
        sets = compute_sets_single_song(song, [motifs[-1]])
        curMax = 0.
        curArgmax = 0
        # for each song of the corpus1, compute similarity with currently studied song
        for s in range(N1):
            nov = compare_to_sets_single_song(getSong(corpus1, s), [motifs[-1]], sets)
            coincidences = (1. - nov) * (len(corpus1["dTseqs"][s]) - motifs[-1] + 1)
            real_sim = coincidences / (lenSong - motifs[-1] + 1)
            if real_sim > curMax:
                curMax = real_sim
                curArgmax = s
    ...
    best = np.argsort(novelties[:,2])[-5:]
    print("Most original songs at motif size {}:".format(motifs[2]))
    for i in best[::-1]:
        print("Generated song {} : novelty {}".format(i, novelties[i, 2]))

    best = np.argsort(novelties[:, -1])[-5:]
    print("Most original songs at motif size {}:".format(motifs[-1]))
    for i in best[::-1]:
        print("Generated song {} : novelty {}".format(i, novelties[i, -1]))
...
def compare_to_sets(corpus, motifs, sets):
    ...
    Returns an ndarray (number_of_songs, number_of_motif_sizes)
    """
    whole_corpus = isinstance(corpus, dict)
    num_songs = len(corpus["dTseqs"]) if whole_corpus else len(corpus)
